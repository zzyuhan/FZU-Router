void ILP_GetCrossover_self(const int& pop_ind) {  //
   ...
   //     }
        ...
        Point p1, p2;
        for (i = r1; i <= r2; ++i) {
            if (pop_ilp[pop_ind].sol[i] != pbest_ilp[pop_ind].sol[i]) { //???? study carefully the process????
...
void ILP_GetCrossover_self2(const int& pop_ind) {  //
	srand((unsigned)time(NULL));
	double c = rand() / (RAND_MAX + 1.0);
	int sol_size = pop_ilp[pop_ind].sol.size();
	if (c < c1_now_ilp) {
...
void ILP_GetCrossover_self3(const int& pop_ind) {  //
	srand((unsigned)time(NULL));
	double c = rand() / (RAND_MAX + 1.0);
	int sol_size = pop_ilp[pop_ind].sol.size();
	if (c < c1_now_ilp) {
